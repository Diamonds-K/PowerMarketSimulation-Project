...
MarketResult QuadraticClearing::clear(const MarketInput& input) {
    if (input.mode() != MarketMode::Quadratic) {
        return makeInfeasibleResult(input, "QuadraticClearing 只能处理二次曲线报价模式");
    }
    ...
    for (const auto& generator : input.generators()) {
        ...
        if (unit.a <= 0.0) {
            return makeInfeasibleResult(input, "二次项系数 a 必须大于 0");
        }
    ...
    if (demand + kTolerance < sumPMin || demand > sumPMax + kTolerance) {
        return makeInfeasibleResult(input, "总需求 QD 超出 [ΣPmin, ΣPmax] 范围，判定不可行");
    }
    ...
    for (const auto& consumer : input.consumers()) {
        ...
        // 二次模式需求刚性：申报量 = 成交电量 = 固定需求 QD。
        consumerResult.declaredDemandMw = consumer.fixedDemandMw();
