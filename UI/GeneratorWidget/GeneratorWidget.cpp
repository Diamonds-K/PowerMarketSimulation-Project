    ...
    : QWidget(parent)
{
    ...
    paramLayout->addWidget(new QLabel(QStringLiteral("电厂名称:"), paramGroup));
    nameEdit_ = new QLineEdit(paramGroup);
    nameEdit_->setPlaceholderText(QStringLiteral("如：XX 电厂"));
    ...
    auto *resultGroup = new QGroupBox(QStringLiteral("本机组出清结果（当前时段）"), this);
    auto *resultLayout = new QGridLayout(resultGroup);
    const auto addResultRow = [&resultLayout, resultGroup](int row,
                                                           const QString &caption,
                                                           QLabel *&value) {
        resultLayout->addWidget(new QLabel(caption, resultGroup), row, 0);
        value = new QLabel(QStringLiteral("-"), resultGroup);
        QFont boldFont = value->font();
        boldFont.setBold(true);
        value->setFont(boldFont);
        resultLayout->addWidget(value, row, 1);
    };
    addResultRow(0, QStringLiteral("统一出清电价 (元/MWh)："), clearingPriceLabel_);
    addResultRow(1, QStringLiteral("中标出力 (MW)："), outputLabel_);
    addResultRow(2, QStringLiteral("上网收益 (元)："), revenueLabel_);
    addResultRow(3, QStringLiteral("报价成本 (元)："), costLabel_);
    addResultRow(4, QStringLiteral("净利润 (元)："), profitLabel_);
    resultStatusLabel_ = new QLabel(
        QStringLiteral("提交申报或切换时段后，自动按当前市场模式出清并刷新结果。"), resultGroup);
    resultStatusLabel_->setWordWrap(true);
    resultLayout->addWidget(resultStatusLabel_, 5, 0, 1, 2);
...
{
    saveCurrent();
    refreshClearingResult();
}

void GeneratorWidget::setRunner(SimulationRunner *runner)
{
    runner_ = runner;
    refreshClearingResult();
}

void GeneratorWidget::refreshClearingResult()
{
    if (units_.empty()) {
        return;
    }
    ...
    // 用当前时段、当前市场模式的完整市场数据出清，再取出本机组那一行。
    ...
    if (!market.feasible()) {
        ...
        outputLabel_->setText(QStringLiteral("-"));
        revenueLabel_->setText(QStringLiteral("-"));
        costLabel_->setText(QStringLiteral("-"));
        profitLabel_->setText(QStringLiteral("-"));
        resultStatusLabel_->setText(QStringLiteral("第 %1 时段出清不可行：%2")
                                        .arg(currentSlotIndex_ + 1)
    ...
    const GeneratorSettlement *mine = nullptr;
    for (const GeneratorSettlement &settlement : slot.settlement.generatorSettlements()) {
        if (settlement.generatorId == unit.id.toStdString()) {
            mine = &settlement;
        }
    }
    if (mine == nullptr) {
        ...
        profitLabel_->setText(QStringLiteral("-"));
        resultStatusLabel_->setText(
            QStringLiteral("未找到本机组的结算明细，请检查机组 ID 是否与申报一致。"));
        return;
    }

    outputLabel_->setText(formatNumber(mine->outputMw, 3) + QStringLiteral(" MW"));
    revenueLabel_->setText(formatNumber(mine->revenueYuan, 2) + QStringLiteral(" 元"));
    costLabel_->setText(formatNumber(mine->costYuan, 2) + QStringLiteral(" 元"));
    profitLabel_->setText(formatNumber(mine->profitYuan, 2) + QStringLiteral(" 元"));
    resultStatusLabel_->setText(
        QStringLiteral("第 %1 时段 · %2 · 市场出清电量 %3 MW · 市场缺额 %4 MW · "
                       "全场总收益 %5 元（按时段 0.25 h 折算）")
            .arg(currentSlotIndex_ + 1)
            .arg(runner_->quadraticMode() ? QStringLiteral("二次曲线模式")
                                          : QStringLiteral("分段报价模式"))
            .arg(formatNumber(market.clearingVolumeMw(), 2))
            .arg(formatNumber(market.shortageMw(), 2))
            .arg(formatNumber(slot.settlement.totalRevenueYuan(), 2)));
}
...
void GeneratorWidget::saveCurrentToSlot(int unitIndex, int slotIndex)
{
    ...
    if (!typedId.isEmpty()) {
        if (idExists(typedId, unitIndex)) {
            resultStatusLabel_->setText(QStringLiteral("机组 ID 重复，未保存"));
            return;
        }
        unit.id = typedId;
    }
...
QString GeneratorWidget::unitId(int index) const
{
    if (index < 0 || index >= static_cast<int>(units_.size())) {
        return QString();
    }
...
QString GeneratorWidget::unitName(int index) const
{
    if (index < 0 || index >= static_cast<int>(units_.size())) {
        return QString();
    }
    const GeneratorUnitData &unit = units_[static_cast<std::size_t>(index)];
                                        ...
                                        QString *errorMessage)
{
    ...
    // 同一机组同一时段最多 10 段（与 BidValidator 的段数上限一致）。
    ...
    for (const CSVReader::Row &row : rows) {
        ...
        if (segmentCount[countKey] >= 10) {
            errors << QStringLiteral("机组 %1 时段 %2 超过 10 段，已忽略多余段")
                          .arg(record.id)
                          .arg(record.timeSlot);
            continue;
        }
