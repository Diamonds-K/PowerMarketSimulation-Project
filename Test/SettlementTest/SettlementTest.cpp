...
void testPiecewiseSettlement() {
    ...
    // 成交 70 MW = Pmin 20 + 增量 50，统一按 250 元/MWh 结算。
    ...
    // 用户账单三栏：申报 80、成交 70、缺额 10。
    assert(settlement.consumerSettlements().size() == 1);
    assert(near(settlement.consumerSettlements()[0].declaredDemandMw, 80.0));
    assert(near(settlement.consumerSettlements()[0].clearedDemandMw, 70.0));
    assert(near(settlement.consumerSettlements()[0].shortageMw, 10.0));
}
...
void testMultiAgentSettlement() {
    ...
    // 每个用户都要能看到申报、成交与缺额，且缺额之和等于市场总缺额。
    double declaredSum = 0.0;
    double shortageSum = 0.0;
    for (const ConsumerSettlement &consumer : settlement.consumerSettlements()) {
        assert(consumer.clearedDemandMw <= consumer.declaredDemandMw + 1e-9);
        assert(near(consumer.shortageMw,
                    consumer.declaredDemandMw - consumer.clearedDemandMw,
                    1e-9));
        declaredSum += consumer.declaredDemandMw;
        shortageSum += consumer.shortageMw;
    }
    assert(near(declaredSum, 110.0));
    assert(near(shortageSum, result.shortageMw(), 1e-9));
}
