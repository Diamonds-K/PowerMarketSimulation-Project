...
SettlementResult SettlementEngine::settle(const MarketInput& input,
    ...
    for (size_t i = 0; i < input.consumers().size(); ++i) {
        ...
        // 申报量口径与出清算法一致：二次模式为固定需求 QD，分段模式为各段增量电量之和。
        const double declaredMw = input.mode() == MarketMode::Quadratic
                                      ? consumer.fixedDemandMw()
                                      : consumer.bidSheet().totalIncrementMw();
        ...
        settlementItem.declaredDemandMw = declaredMw;
        settlementItem.clearedDemandMw = clearedMw;
        settlementItem.shortageMw = std::max(0.0, declaredMw - clearedMw);
