...
│   ├── SimulationRunner/SimulationRunner.h
│   ├── SimulationRunner/SimulationRunner.cpp
...
└── Test/
    ...
    ├── ValidationTest/ValidationTest.cpp
    └── TradingCenterTest/TradingCenterTest.cpp
```

> 说明：需求文档中“交易中心”承担“汇总申报、组织出清、发布结果、完成结算”的职责，且 UI 层不得直接调用算法，因此增加顶层 `TradingCenter` 模块作为**应用服务门面**。它不改变既定的七层方向，只是补上 UI 与算法之间的编排层。

> `UI/SimulationRunner` 是 **UI 层内部**的编排助手（不是新分层）：它持有 `TradingCenter`，负责把发电侧与用户侧页面录入的申报组装成 `MarketInput`，供三个页面共用，避免各页面各自复制组装逻辑。除它以外，UI 不得出现第二处 `MarketInput` 组装代码。
...
- `SimulationRunner`：UI 内部编排助手，组装 `MarketInput` 并调用 `TradingCenter`，供三个页面共用。

UI 不得直接调用算法，只能调用 `TradingCenter`。

三个页面都必须能展示**本角色自己的**结果：发电侧展示中标出力与收入，用户侧展示成交电量与账单，交易中心展示全体申报、出清价与全体收支。
...
- `ValidationTest`：报价校验与全局可行性用例；
- `TradingCenterTest`：96 时段端到端仿真用例。
...
| UI | Model、TradingCenter、Data（仅通过 Data 层类读写 CSV/SQLite）、Qt Widgets | 直接调用 ClearingEngine / SettlementEngine / BidValidator |
| UI 内部 | 页面之间只通过 `SimulationRunner` 与信号槽协作 | 复制 `MarketInput` 组装逻辑 |
| Test | Model、Validation、Clearing、Settlement、TradingCenter | UI（默认） |
...
```cpp
// UI 层唯一允许组装 MarketInput 的地方：SimulationRunner
pms::SimulationRunner runner(generatorWidget, consumerWidget);

// 单时段：发电侧/用户侧查看自己的结果
pms::TimeSlotResult slotResult = runner.runSlot(40, /*quadratic=*/false);

// 全天 96 时段
std::vector<pms::TimeSlotResult> results = runner.runAll(false);
```

`SimulationRunner` 内部持有一个 `pms::TradingCenter`，页面不得绕过它自行组装 `MarketInput`。
...
| `UI/SimulationRunner` | `SimulationRunner` | UI 内部编排助手（组装 `MarketInput`） |
...
| `Test/ValidationTest` | `ValidationTest` | 报价与可行性校验用例 |
| `Test/TradingCenterTest` | `TradingCenterTest` | 96 时段端到端用例 |
...
9. UI 组装 `MarketInput` 只能通过 `UI/SimulationRunner`，页面之间不得互相复制组装逻辑。
10. 不可行结果必须与输入主体等长（零值明细），禁止返回长度不足的明细数组。
