...
ValidationReport BidValidator::validate(const MarketInput& input) {
    ...
    const bool quadraticMarket = input.mode() == MarketMode::Quadratic;
    ...
    for (const auto& generator : input.generators()) {
        report.merge(validateGenerator(generator, quadraticMarket));
    }
    for (const auto& consumer : input.consumers()) {
        report.merge(validateConsumer(consumer, quadraticMarket));
    }
    return report;
}

ValidationReport BidValidator::validateGenerator(const Generator& generator,
                                                bool quadraticMarket) {
    ValidationReport report;
    ...
    report.merge(validateBidSheet(generator.bidSheet(), availableCapacity, true, quadraticMarket));
    return report;
}
...
ValidationReport BidValidator::validateConsumer(const Consumer& consumer,
    ...
    report.merge(validateBidSheet(consumer.bidSheet(), 0.0, false, quadraticMarket));
    return report;
}
...
ValidationReport BidValidator::validateBidSheet(const BidSheet& bidSheet,
    ...
    // 注意：二次曲线模式下用户侧只申报固定需求 QD，因此不要求分段报价段。
    if (quadraticMarket || bidSheet.mode() == BidSheet::Mode::Quadratic) {
